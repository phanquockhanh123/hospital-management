              ...
              <form action="{{ route('beds.index') }}" method="GET">
                <div class="input-group mb-2">
                  <div class="input-group mb-3">
                    <input type="text" class="form-control" placeholder="Search Beds" name="search">
                ...
                <div class="card-body">
                  ...
                  <table id="example2" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      @foreach ($beds as $bed)
                      <tr>
                        <td><a href="{{ route('beds.show', $bed->id) }}">{{ $bed->bed_code }}</a></td>
                        <td>{{ $bed->name }}</td>
                        <td>{{ $bed->bed_type }}</td>
                        ...
                        <td>
                          <div class="btn-group">
                            ...
                            <button type="button" class="btn btn-danger" data-toggle="modal"
                              data-target="#deleteModal{{ $bed->id }}" style="color: red;">
                        ...
                        <!-- Modal -->
                        <div class="modal fade" id="deleteModal{{ $bed->id }}" tabindex="-1" role="dialog"
                          aria-labelledby="deleteModalLabel{{ $bed->id }}" aria-hidden="true">
                          <div class="modal-dialog" role="document">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h5 class="modal-title" id="deleteModalLabel{{ $bed->id }}">Xóa loại giường</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                  <span aria-hidden="true">&times;</span>
                                </button>
                              </div>
                              <div class="modal-body">
                                Bạn có chắc chắn muốn xóa loại giường "{{ $bed->name }}" không? Hành động này không
                                thể hoàn tác!
                              </div>
