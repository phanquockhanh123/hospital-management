              ...
              <form action="{{ route('addmission_patients.index') }}" method="GET">
                <div class="input-group mb-2">
                  <div class="input-group mb-3">
                    <input type="text" class="form-control" placeholder="Search addmission patients" name="search">
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="card">
                ...
                <!-- /.card-header -->
                <div class="card-body">
                  @if ($addmission_patients->isEmpty())
                  ...
                  <table id="example2" class="table table-bordered table-hover">
                    ...
                    <tbody>
                      @foreach ($addmission_patients as $addmission_patient)
                      <tr>
                        <td>
                          <a href="{{ route('addmission_patients.show', $addmission_patient->id) }}">{{ $addmission_patient->id }}</a>
                        </td>
                        ...
                        <td>
                          @if($addmission_patient->status == 0)
                          <span class="text-danger">Đang chờ xử lý</span>
                          @endif
                          @if($addmission_patient->status == 1)
                        ...
                        <td>
                          <div class="btn-group">
                            ...
                            <button type="button" class="btn btn-danger" data-toggle="modal"
                              data-target="#deleteModal{{ $addmission_patient->id }}" style="color: red;">
                        ...
                        <!-- Modal -->
                        <div class="modal fade" id="deleteModal{{ $addmission_patient->id }}" tabindex="-1" role="dialog"
                          aria-labelledby="deleteModalLabel{{ $addmission_patient->id }}" aria-hidden="true">
                          <div class="modal-dialog" role="document">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h5 class="modal-title" id="deleteModalLabel{{ $addmission_patient->id }}">Xóa bệnh nhân</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                  <span aria-hidden="true">&times;</span>
                                </button>
                              </div>
                              <div class="modal-body">
                                Bạn có chắc chắn muốn xóa bệnh án "{{ $addmission_patient->id }}" không? Hành động này không
                                thể hoàn tác!
                              </div>
